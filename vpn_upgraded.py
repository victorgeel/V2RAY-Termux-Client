...
def normalize_protocol_config(link: str) -> dict:
    result = {
        "protocol": "unknown",
        "address": None,
        "port": None,
    ...
    try:
        ...
        elif link.startswith(("vless://", "trojan://", "hy2://", "hysteria://")):
            u = urlparse(link)
            qs = parse_qs(u.query)
            result.update({
                "protocol": u.scheme,
                ...
                "security": qs.get("encryption", [None])[0],
...
async def fetch_subscription(url: str, timeout: int = 10) -> list[str]:
    try:
        ...
        if proc.returncode != 0:
            return []
        return decode_base64_lines(out.decode(errors="ignore"))
    except Exception:
        return []

def decode_base64_lines(data: str) -> list[str]:
    try:
...
# --- xray_config.py ---
def generate_xray_config(vmess: dict, socks_port=10808, http_port=10809) -> dict:
    try:
        outbound = {
            "protocol": "vmess",
            "settings": {
                "vnext": [{
                    "address": vmess.get("address"),
                    "port": int(vmess.get("port")),
                    "users": [{
                        "id": vmess.get("id"),
                        "alterId": int(vmess.get("aid", 0)),
                        "security": vmess.get("security", "auto")
                    }]
                }]
            },
            "streamSettings": {
